...
public static class Lawyer
{
    ...
    public static readonly string[] ChangeRoles =
    {
        ...
        "Role.Mayor",
        "Role.Doctor",
     //   CustomRoles.Dictator.ToString(),CustomRoles.Bodyguard.ToString(),CustomRoles.Opportunist.ToString(),CustomRoles.Crewmate.ToString(),CustomRoles.Jester.ToString(),CustomRoles.Convict.ToString(),CustomRoles.Celebrity.ToString(),CustomRoles.Mayor.ToString(),CustomRoles.Doctor.ToString(),
    };
    ...
    //        .SetValueFormat(OptionFormat.Multiplier);
        ...
        CanTargetCoven = BooleanOptionItem.Create(Id + 13, "LawyerCanTargetCoven", false, TabGroup.NeutralRoles, false).SetParent(CustomRoleSpawnChances[CustomRoles.Lawyer]);
        CanTargetJester = BooleanOptionItem.Create(Id + 14, "LawyerCanTargetJester", false, TabGroup.NeutralRoles, false).SetParent(CustomRoleSpawnChances[CustomRoles.Lawyer]);
        ...
        ChangeRolesAfterTargetKilled = StringOptionItem.Create(Id + 17, "LawyerChangeRolesAfterTargetKilled", ChangeRoles, 1, TabGroup.NeutralRoles, false).SetParent(CustomRoleSpawnChances[CustomRoles.Lawyer]);
        QSR.SkillCooldown = FloatOptionItem.Create(Id + 18, "QSRSkillCooldown", new(2.5f, 900f, 2.5f), 20f, TabGroup.NeutralRoles, false).SetParent(CustomRoleSpawnChances[CustomRoles.Lawyer])
            .SetValueFormat(OptionFormat.Seconds);
        QSR.SkillLimitOpt = IntegerOptionItem.Create(Id + 19, "QSRSkillLimit", new(1, 990, 1), 3, TabGroup.NeutralRoles, false).SetParent(CustomRoleSpawnChances[CustomRoles.Lawyer])
    ...
    public static void Add(byte playerId)
    {
        ...
        if (AmongUsClient.Instance.AmHost)
        {
            ...
            foreach (var target in Main.AllPlayerControls)
            {
                ...
                else if (target.Is(CustomRoleTypes.Neutral) && !target.IsNeutralKiller() && !target.Is(CustomRoles.Jester) && !target.Is(CustomRoles.Arsonist) && !target.Is(CustomRoles.PlatonicLover) && !target.Is(CustomRoles.Totocalcio)) continue;
