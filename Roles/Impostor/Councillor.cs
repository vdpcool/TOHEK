    ...
    public static bool MurderMsg(PlayerControl pc, string msg, bool isUI = false)
    {
        ...
        else if (operate == 2)
        {
            ...
            if (target != null)
            {
                ...
                {
                    if (!isUI) Utils.SendMessage(GetString("LaughToWhoMurderSelf"), pc.PlayerId, Utils.ColorString(Color.cyan, GetString("MessageFromK")));
                    else pc.ShowPopUp(Utils.ColorString(Color.cyan, GetString("MessageFromK")) + "\n" + GetString("LaughToWhoMurderSelf"));
