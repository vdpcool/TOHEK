...
internal class ChatCommands
{
    ...
    {
        var friendCodesFilePath = @"./TOHEK_DATA/Moderators.txt";
        var friendCodes = File.ReadAllLines(friendCodesFilePath);
        return friendCodes.Contains(friendCode);
    }
    // Function to check if a player is a tester
    public static bool IsPlayerTester(string friendCode)
    {
        var friendCodesFilePath = @"./TOHEK_DATA/Testers.txt";
        var friendCodes = File.ReadAllLines(friendCodesFilePath);
        return friendCodes.Contains(friendCode);
    }
    ...
    public static bool Prefix(ChatController __instance)
    {
        ...
        if (AmongUsClient.Instance.AmHost)
        {
            ...
            switch (args[0])
            {
                ...
                case "/rename":
                    ...
                    if (newName.Length < 1)
                        ...
                        Utils.SendMessage("Name is too long. Maximum length is 100 characters.", PlayerControl.LocalPlayer.PlayerId);
                    else Main.nickName = newName;
                    break;
                ...
                case "/up":
                    ...
                    if (!Options.EnableUpMode.GetBool())
                    {
                        ...
                        Utils.SendMessage(string.Format(GetString("Message.TwitchPlanDisabled"), GetString("EnableTwitchPlan")));
                        break;
                    }
            ...
            canceled = true;
                ...
                // Prevent testers from kicking other testers
                if (IsPlayerTester(kickedPlayer.FriendCode))
                {
                    Utils.SendMessage(GetString("KickCommandKickMod"), PlayerControl.LocalPlayer.PlayerId);
                    break;
                }
    ...
    public static string FixRoleNameInput(string text)
    {
        ...
        return text switch
        {
            ...
            "GrudgeSheriff" => GetString("GrudgeSheriff"),
            "SillySheriff" => GetString("SillySheriff"),
    ...
    public static void SendRolesInfo(string input, byte playerId, bool isDev = false, bool isUp = false)
    {
        ...
        if (!GetRoleByName(input, out var role))
        {
            if (isUp) Utils.SendMessage(GetString("Message.YTPlanCanNotFindRoleThePlayerEnter"), playerId);
            if (isUp) Utils.SendMessage(GetString("Message.TwitchPlanCanNotFindRoleThePlayerEnter"), playerId);
        ...
        if ((isDev || isUp) && GameStates.IsLobby)
        {
            ...
            if (isUp)
            {
                ...
                if (canSpecify) Utils.SendMessage(string.Format(GetString("Message.TwitchPlanSelected"), roleName), playerId);
                else Utils.SendMessage(string.Format(GetString("Message.TwitchPlanSelectFailed"), roleName), playerId);
                return;
            }
        }
    ...
    public static void OnReceiveChat(PlayerControl player, string text, out bool canceled)
    {
        ...
        switch (args[0])
        {
            ...
            case "/id":
                ...
                if (Options.ApplyTesterList.GetValue() == 0 || !IsPlayerTester(player.FriendCode)) break;
            ...
            case "/kick":
                ...
                if (IsPlayerTester(kickedPlayer.FriendCode))
                {
                    Utils.SendMessage(GetString("KickCommandKickTest"), player.PlayerId);
                    break;
                }
