...
class AddTasksFromListPatch
{
        ...
        [HarmonyArgument(4)] Il2CppSystem.Collections.Generic.List<NormalPlayerTask> unusedTasks)
    {
        ...
        for (var i = 0; i < unusedTasks.Count; i++)
        {
            ...
            if (task.TaskType == TaskTypes.FixWeatherNode && Options.DisableFixWeatherNode.GetBool()) disabledTasks.Add(task);//FixWeatherNode task
        }
...
class RpcSetTasksPatch
{
    ...
    [HarmonyArgument(1)] ref Il2CppStructArray<byte> taskTypeIds)
    {
        ...
        //管理员，摆烂人没有任务
        if (pc.Is(CustomRoles.SuperPowers) ||  pc.Is(CustomRoles.GM) || pc.Is(CustomRoles.Needy) || Options.CurrentGameMode == CustomGameMode.SoloKombat || Options.CurrentGameMode == CustomGameMode.HotPotato)
        {
            hasCommonTasks = false;
            NumShortTasks = 0;
            NumLongTasks = 0;
        ...
        //超能力者拥有两个任务
        if (pc.Is(CustomRoles.SuperPowers))
        {
            hasCommonTasks = false;
            NumShortTasks = 2;
            NumLongTasks = 0;
