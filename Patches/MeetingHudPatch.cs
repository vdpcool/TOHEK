...
class CheckForEndVotingPatch
{
    public static bool Prefix(MeetingHud __instance)
    {
        ...
        try
        {
            ...
            for (var i = 0; i < __instance.playerStates.Length; i++)
            {
                ...
                if (CheckRole(ps.TargetPlayerId, CustomRoles.Jester) && Options.JesterHideVote.GetBool()) continue;
                ...
                if (CheckRole(ps.TargetPlayerId, CustomRoles.Monarch) && !Monarch.KnightHideVote.GetBool()) //Monarch
                {
                    for (var i2 = 0; i2 < Monarch.KnightAdditionalVote.GetFloat(); i2++)
                    {
                        statesList.Add(new MeetingHud.VoterState()
                        {
                            VoterId = ps.TargetPlayerId,
    ...
    private static void ConfirmEjections(GameData.PlayerInfo exiledPlayer)
    {
        ...
        switch (Options.CEMode.GetInt())
        {
            ...
            case 1:
                ...
                else if (player.GetCustomRole().IsNeutral() && !player.GetCustomRole().IsCoven() && !player.Is(CustomRoles.Parasite) && !player.Is(CustomRoles.Crewpostor) && !player.Is(CustomRoles.Convict))
   ...
   /*     if (crole == CustomRoles.Jester)
    ...
    private static void RevengeOnExile(byte playerId, PlayerState.DeathReason deathReason)
    {
        ...
        if (player.Is(CustomRoles.Nekomata))
        {
    ...
    internal static void TryAddAfterMeetingDeathPlayers(object poisoning, byte targetId)
    {
    ...
    public static Dictionary<byte, int> CustomCalculateVotes(this MeetingHud __instance)
    {
        ...
        for (int i = 0; i < __instance.playerStates.Length; i++)
        {
            ...
            if (ps.VotedFor is not 252 and not byte.MaxValue and not 254)//该玩家面板里是否投了该玩家
            {
                ...
                if (CheckForEndVotingPatch.CheckRole(ps.TargetPlayerId, CustomRoles.Monarch)
                    && ps.TargetPlayerId != ps.VotedFor
                    ) VoteNum += 1;
                if (CheckForEndVotingPatch.CheckRole(ps.TargetPlayerId, CustomRoles.Knighted)
                    && ps.TargetPlayerId != ps.VotedFor
                    ) VoteNum += Monarch.KnightAdditionalVote.GetInt();
...
{
    public static PlayerControl pc { get; private set; }
    ...
    public static void Postfix(MeetingHud __instance)
    {
        ...
        foreach (var pva in __instance.playerStates)
        {
            ...
            roleTextMeeting.enabled =
                ...
                (pc.Is(CustomRoles.Snitch) && Snitch.SnitchRevealWhenDoneTasks.GetBool() && pc.AllTasksCompleted()) ||
            ...
            {
                if ((PlayerControl.LocalPlayer.Is(CustomRoles.TaskManager)) && PlayerControl.LocalPlayer == pc)
                    roleTextMeeting.text += $" {Utils.GetTMtaskCountText(pc)}";
            }
            if (Options.IsSyncColorMode && !(PlayerControl.LocalPlayer.Is(CustomRoles.TaskManager)) && PlayerControl.LocalPlayer.IsAlive()) roleTextMeeting.enabled = false;
        }
        ...
        foreach (var pva in __instance.playerStates)
        {
            ...
            if (Options.GuesserMode.GetBool())
            {
                ...
                if (Options.ImpostorsCanGuess.GetBool() && seer.GetCustomRole().IsImpostor() && !seer.Is(CustomRoles.Councillor))
                    if (!seer.Data.IsDead && !target.Data.IsDead)
                    pva.NameText.text = Utils.ColorString(Utils.GetRoleColor(seer.GetCustomRole()), target.PlayerId.ToString()) + " " + pva.NameText.text;
                if (Options.CovenMembersCanGuess.GetBool() && seer.GetCustomRole().IsCoven())
                    if (!seer.Data.IsDead && !target.Data.IsDead)
                    pva.NameText.text = Utils.ColorString(Utils.GetRoleColor(seer.GetCustomRole()), target.PlayerId.ToString()) + " " + pva.NameText.text;
