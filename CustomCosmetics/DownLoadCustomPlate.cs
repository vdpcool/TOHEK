...
public static class DownLoadClass
{
    ...
    {
        //_ = CredentialsPatch.LogoPatch.FetchBoosters();
        if (running)
            return;
        IsEndDownload = false;
        Directory.CreateDirectory(Path.GetDirectoryName(Application.dataPath) + @"\TOHEK\");
        Directory.CreateDirectory(Path.GetDirectoryName(Application.dataPath) + @"\TOHEK\CustomPlatesChache\");
        Main.Logger.LogInfo("[CustomPlate:Download] ダウンロード開始");
    ...
    {
        fetchs.Add(repo);
        Main.Logger.LogInfo("[CustomPlate:Download] ダウンロード開始:" + repo);
        HttpClient http = new();
        ...
        try
        {
            ...
            string filePath = Path.GetDirectoryName(Application.dataPath) + @"\TOHEK\CustomPlatesChache\";
            MD5 md5 = MD5.Create();
        ...
        Main.Logger.LogInfo("[CustomPlate:Download] ダウンロード終了:" + repo);
        fetchs.Remove(repo);
        if (fetchs.Count <= 0)
        {
            IsEndDownload = true;
