...
public static class DownLoadClassVisor
{
    ...
    public static void Load()
    {
        ...
        Directory.CreateDirectory(Path.GetDirectoryName(Application.dataPath) + @"\TOHEK\CustomVisorsChache\");
        Main.Logger.LogInfo("[CustomVisor:Download] バイザーダウンロード開始");
    ...
    {
        fetchs.Add(repo);
        Main.Logger.LogInfo("[CustomVisor:Download] バイザーダウンロード開始:" + repo);
        HttpClient http = new();
        ...
        try
        {
            ...
            string filePath = Path.GetDirectoryName(Application.dataPath) + @"\TOHEK\CustomVisorsChache\";
            MD5 md5 = MD5.Create();
        ...
        Main.Logger.LogInfo("[CustomVisor:Download] バイザーダウンロード終了:" + repo);
        fetchs.Remove(repo);
        if (fetchs.Count <= 0)
        {
            IsEndDownload = true;
