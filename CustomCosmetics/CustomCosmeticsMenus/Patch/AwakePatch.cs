...
class AwakePatch
{
    ...
    public static void Postfix(PlayerCustomizationMenu __instance)
    {
        ...
        Main.Logger.LogInfo(ClosetTabButton.name + ":" + PresetTabButton.name);
        int i = 0;
        foreach (TabButton button in __instance.Tabs)
        {
            Main.Logger.LogInfo(button.Button.transform.parent.parent.name);
            if (i > 1)
            {
        ...
        ClosetIcon.GetComponent<SpriteRenderer>().sprite = Utils.LoadSprite("TOHE.Resources.Images.Cosmetics.ClosetButton.png", 115f);
        PresetIcon.GetComponent<SpriteRenderer>().sprite = Utils.LoadSprite("TOHE.Resources.Images.Cosmetics.PresetButton.png", 115f);
