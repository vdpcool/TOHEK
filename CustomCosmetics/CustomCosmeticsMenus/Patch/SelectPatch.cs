...
using HarmonyLib;
using static TOHE.CustomCosmetics.CustomCosmeticsMenus.Patch.ObjectData;

namespace TOHE.CustomCosmetics.CustomCosmeticsMenus.Patch;
...
class SelectPatch
{
    ...
    public static ClosetPresetData GetData(int index = -1)
    {
        ...
        ClosetPresetData data = !ClosetPresetDataDictionary.ContainsKey(index)
            ? (new()
            {
                BodyColor = Main.Instance.Config.Bind("ClosetPreset_" + index.ToString(), "BodyColor", (byte)0),
                Hat = Main.Instance.Config.Bind("ClosetPreset_" + index.ToString(), "Hat", ""),
                Visor = Main.Instance.Config.Bind("ClosetPreset_" + index.ToString(), "Visor", ""),
                Skin = Main.Instance.Config.Bind("ClosetPreset_" + index.ToString(), "Skin", ""),
                NamePlate = Main.Instance.Config.Bind("ClosetPreset_" + index.ToString(), "NamePlate", ""),
                Pet = Main.Instance.Config.Bind("ClosetPreset_" + index.ToString(), "Pet", "")
            })
