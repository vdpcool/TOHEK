    ...
    public static bool GuesserMsg(PlayerControl pc, string msg, bool isUI = false)
    {
        ...
        else if (operate == 2)
        {
            ...
            if (target != null)
            {
                ...
                if (Snitch.SnitchRevealWhenDoneTasks.GetBool())
                {
                    if (target.Is(CustomRoles.Snitch) && target.AllTasksCompleted())
                    {
                        if (!isUI) Utils.SendMessage(GetString("GuessSnitch"), pc.PlayerId);
                        else pc.ShowPopUp(GetString("GuessSnitch"));
                        return true;
                    }
                }
                ...
                /*if (role == CustomRoles.LastImpostor || role == CustomRoles.Mare)
                {
         ...
         /*       if ((pc.Is(CustomRoleTypes.Impostor) && target.Is(CustomRoleTypes.Impostor) && !Options.ImpCanGuessImp.GetBool()) && Options.GuesserMode.GetBool())
                ...
                else if (pc.Is(CustomRoles.EvilGuesser) && target.Is(CustomRoleTypes.Impostor) && (role.IsImpostor() || role == CustomRoles.SchrodingerCat && SchrodingerCat.isimp == true) && !Options.EGCanGuessImp.GetBool()) 
                {
                    if (pc.Is(CustomRoles.DoubleShot) && !DoubleShot.IsActive.Contains(pc.PlayerId))
                    {
                        DoubleShot.IsActive.Add(pc.PlayerId);
