...
public static class Utils
{
    ...
    {
        if (seer.Is(CustomRoles.GM) || seer.Is(CustomRoles.Seer) || seer.Is(CustomRoles.VIP)) return true;
        if (seer.Data.IsDead || killer == seer || target == seer) return false;
        if (seer.Is(CustomRoles.EvilTracker)) return EvilTracker.KillFlashCheck(killer, target);
        foreach (var subRole in Main.PlayerStates[target.PlayerId].SubRoles)
        {
    ...
    public static bool HasTasks(GameData.PlayerInfo p, bool ForRecompute = true)
    {
      ...
      //      case CustomRoles.Chameleon:
            ...
            case CustomRoles.LoveCutter:
                if ((Options.LoveCutterKnow.GetBool() && ForRecompute) || !Options.LoveCutterKnow.GetBool())
                    hasTasks = false;
                break;
    ...
    public static bool CanBeMadmate(this PlayerControl pc)
    {
        ...
        && !(
            ...
            (pc.Is(CustomRoles.SillySheriff) && !Options.SillySheriffCanBeMadmate.GetBool()) ||
    ...
    public static string GetProgressText(byte playerId, bool comms = false)
    {
        ...
        switch (role)
        {
            ...
            case CustomRoles.Sheriff:
                ...
                break;
            case CustomRoles.SillySheriff:
                ProgressText.Append(SillySheriff.GetShotLimit(playerId));
                break;
            case CustomRoles.LoveCutter:
                int cut = Main.LoveCutterKilledCount[playerId];
                int win = Options.VictoryCutCount.GetInt();
                ProgressText.Append(ColorString(GetRoleColor(CustomRoles.LoveCutter), $"({cut}/{win})"));
                goto default;
            case CustomRoles.GrudgeSheriff:
                ProgressText.Append(GrudgeSheriff.GetShotLimit(playerId));
                goto default;
            ...
            case CustomRoles.ElectOfficials:
                ...
                break;
            case CustomRoles.QX:
                int QXTimes = Options.QXShields.GetInt();
                ProgressText.Append(ColorString(GetRoleColor(CustomRoles.QX), $"{QXTimes}"));
                break;
            default:
                //タスクテキスト
                ...
                if (taskState.hasTasks)
                {
                    ...
                    var TaskCompleteColor = Utils.HasTasks(info) ? Color.green : GetRoleColor(role).ShadeColor(0.5f); //タスク完了後の色
    ...
    public static string GetTMtaskCountText(PlayerControl pc)
    {
        var Comms = false;
        ...
        if ((GameStates.IsMeeting || Main.PlayerStates[pc.PlayerId].IsDead
            || (pc.Is(CustomRoles.TaskManager) && Options.TaskManagerSeeNowtask.GetBool())
            ) && !Comms)
            nowtask = $"{completetask}";
        ProgressText = ColorString(Color.cyan, $"({nowtask}/{alltask})");
        return ProgressText;
    }
    private static (int,int) GetTasksState()
    {
        ...
        {
            var taskState = Main.PlayerStates?[pc.PlayerId].GetTaskState();
    ...
    public static void ShowAllActiveSettings(byte PlayerId = byte.MaxValue)
    {
        ...
        private static bool NotShowOption(string optionName)
        {
    ...
    public static byte MsgToColor(string text, bool isHost = false)
    {
        ...
        switch (text)
        {
            ...
            case "18": case "隐藏": case "fortegreen": color = 18; break;
        }
    ...
    public static void CheckLoveCutterWin(GameData.PlayerInfo LoveCutter)
    {
        if (!AmongUsClient.Instance.AmHost) return;
        if (Main.LoveCutterKilledCount[LoveCutter.PlayerId] >= Options.VictoryCutCount.GetInt())
        {
            foreach (var pc in PlayerControl.AllPlayerControls)
            {
                if (pc.Is(CustomRoles.LoveCutter))
                {
            ...
            CustomWinnerHolder.WinnerIds.Add(LoveCutter.PlayerId);
        }
    }
    public static void SendMessage(string text, byte sendTo = byte.MaxValue, string title = "")
    {
        if (!AmongUsClient.Instance.AmHost) return;
        if (title == "") title = "<color=#ed2f91>" + GetString("DefaultSystemMessageTitle") + "</color>";
        ...
        else
        {
            ...
            if (player.AmOwner)
            {
                if (GameStates.IsOnlineGame || GameStates.IsLocalGame)
                    ...
                    //name = $"<color=#ffc0cb>{GetString("HostText")}</color><color=#baf7ca>★</color>" + name;
            ...
            if (!name.Contains('\r') && player.FriendCode.GetDevUser().HasTag())
                ...
                {
                    SuffixModes.None => name += $"\r\n<size=1.7><color={Main.ModColor}>{GetString("SuffixMode.None")}</color></size>",
                    SuffixModes.TOHE => name += $"\r\n<color={Main.ModColor}>TOHE-K v{Main.PluginVersion}</color>",
                    ...
                    SuffixModes.Myseft => name += $"\r\n<size=1.7><color={Main.ModColor}>{GetString("SuffixModeText.Myseft")}</color></size>",
                    SuffixModes.DoNotKillMePlz => name += $"\r\n<size=1.7><color={Main.ModColor}>{GetString("SuffixModeText.DoNotKillMePlz")}</color></size>",
                    SuffixModes.NoAndroidorIOSPlz => name += $"\r\n<size=1.7><color={Main.ModColor}>{GetString("SuffixModeText.NoAndroidPlz")}</color></size>",
    ...
    public static void ApplyModeratorSuffix(PlayerControl player)
    {
        ...
        else if (Options.ApplyModeratorList.GetValue() == 1)
        {
            if (ChatCommands.IsPlayerModerator(player.FriendCode))
            {
                ...
                name = $"<#ffc0cb>Mod<#baf7ca>★</color></color>" + name;
            }
        }
        if (name != player.name && player.CurrentOutfitType == PlayerOutfitType.Default)
            player.RpcSetName(name);
    }
    public static void ApplyTesterSuffix(PlayerControl player)
    {
        if (player == null) return;
        if (!GameStates.IsLobby) return;
        ...
        else if (Options.ApplyTesterList.GetValue() == 1)
        {
            if (ChatCommands.IsPlayerTester(player.FriendCode))
            {
                
                name = $"<#ffc0cb>Tester<#baf7ca>★</color></color>" + name;
            }
        }
        if (name != player.name && player.CurrentOutfitType == PlayerOutfitType.Default)
            player.RpcSetName(name);
    ...
    public static void NotifyRoles(bool isForMeeting = false, PlayerControl SpecifySeer = null, bool NoCache = false, bool ForceLoop = false, bool CamouflageIsForMeeting = false, bool GuesserIsForMeeting = false)
    {
        ...
        foreach (var seer in seerList)
        {
            ...
            //呪われている人
            TargetMark.Append(Bakery.GetPoisonMark(target, isForMeeting));
            ...
            if (Bakery.IsPoisoned(seer) && isForMeeting && seer.IsAlive())
                SelfMark.Append(ColorString(GetRoleColor(CustomRoles.NBakery), "θ"));
            ...
            Color32 SelfNameColor = default;
            if (Options.IsSyncColorMode && isForMeeting && !(seer.Is(CustomRoles.TaskManager)))
            ...
            foreach (var target in Main.AllPlayerControls)
            {
                ...
                if (seer.Is(CustomRoles.NWitch) &&
                    ...
                    string TargetRoleText =
                        ...
                        (seer.Is(CustomRoles.PlatonicLover) && target.Is(CustomRoles.PlatonicLover) && PlatonicLover.PLoverAddWin.GetBool()) ||
                ...
                if (Options.GuesserMode.GetBool())
                {
                    ...
                    else if (Options.CovenMembersCanGuess.GetBool())
                    {
                        if (seer.IsAlive() && target.IsAlive() && GuesserIsForMeeting)
                        {
                            TargetPlayerName = ColorString(GetRoleColor(seer.GetCustomRole()), target.PlayerId.ToString()) + " " + TargetPlayerName;
    ...
    //            if (seer.Is(CustomRoles.Monarch) && target.Is(CustomRoles.Knighted))
     //               TargetMark.Append(ColorString(GetRoleColor(CustomRoles.Knighted), " 亗"));
                if (seer.Is(CustomRoles.Sidekick) && target.Is(CustomRoles.Sidekick) && Options.SidekickKnowOtherSidekick.GetBool())
                    TargetMark.Append(ColorString(GetRoleColor(CustomRoles.Jackal), " ♥"));
                if (seer.Is(CustomRoles.TaskManager))
                    SelfTaskText = $" {GetTMtaskCountText(seer)}" + SelfTaskText;
             ...
             //   TargetMark.Append(Lawyer.TargetMark(seer, target));
                ...
                if (seer.KnowDeathReason(target))
                    ...
                    TargetDeathReason = $"({ColorString(GetRoleColor(CustomRoles.Autopsy), GetVitalText(target.PlayerId))})";
   ...
   // public static string RemoveHtmlTags(this string str) => Regex.Replace(str, "<[^>]*?>", "");
    ...
    public static PlayerControl target { get; private set; }
