        ...
        if (AmongUsClient.Instance.AmHost)
        {
            ...
            if (player.Is(CustomRoles.Vandalism))
            {
                ...
                if (SysList.Count > 0)
                {
                    ...
                    {
                        Hazel.MessageWriter SabotageFixWriter = AmongUsClient.Instance.StartRpcImmediately(ShipStatus.Instance.NetId, (byte)RpcCalls.RepairSystem, Hazel.SendOption.Reliable, player.GetClientId());
                        SabotageFixWriter.Write((byte)sysType);
                        InnerNet.MessageExtensions.WriteNetObject(SabotageFixWriter, player);
                        AmongUsClient.Instance.FinishRpcImmediately(SabotageFixWriter);

                        foreach (var target in Main.AllPlayerControls)
                        {
                            if (target == player || target.Data.Disconnected) continue;
                            SabotageFixWriter = AmongUsClient.Instance.StartRpcImmediately(ShipStatus.Instance.NetId, (byte)RpcCalls.RepairSystem, Hazel.SendOption.Reliable, target.GetClientId());
                            SabotageFixWriter.Write((byte)sysType);
                            InnerNet.MessageExtensions.WriteNetObject(SabotageFixWriter, target);
                            AmongUsClient.Instance.FinishRpcImmediately(SabotageFixWriter);
                        }
                    }
